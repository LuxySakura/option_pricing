...
def test_zero_spot_price():
    ...
    data = response.json()
    # 断言响应体中包含了正确的错误信息
    assert data["detail"] == "输入数据校验失败: 参数 '标的当前价格' 无效: 输入值必须大于 0.0"
...
def test_zero_strike_price():
    ...
    data = response.json()
    # 断言响应体中包含了正确的错误信息
    assert data["detail"] == "输入数据校验失败: 参数 '行权价格' 无效: 输入值必须大于 0.0"
...
def test_neg_time_to_maturity():
    ...
    data = response.json()
    # 断言响应体中包含了正确的错误信息
    assert data["detail"] == "输入数据校验失败: 参数 '到期时间' 无效: 输入值必须大于或等于 0.0"
